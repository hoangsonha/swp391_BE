...
public interface DiamondRepository extends JpaRepository<Diamond, String>, JpaSpecificationExecutor<Diamond> {
    ...
    @Query(value = "select max(totalPrice) from Diamond")
    public Double getMaxTotalPrice();

    @Query(value = "select max(dimensions) from Diamond")
    public Float getMaxDimensions();

    @Query(value = "select max(carat) from Diamond")
    public Float getMaxCarat();
